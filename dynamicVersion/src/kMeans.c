...
inline static double distance(rVector a, rVector b) {
    return sqrt(pow((a.x - b.x), 2) + pow((a.y - b.y), 2) + pow((a.z - b.z), 2));
}

//копирование текущих центроид в временный буфер для функции сравнения
inline static void arrCopy(rVector *a, rVector *b, size_t l) {
...
inline static short compare(rVector *a, rVector *b, size_t l) {
    short flag = 0;
    for (size_t i = 0; i < l; ++i) {
        if ((fabs(a[i].x - b[i].x) <= 0.00001) && (fabs(a[i].y - b[i].y) <= 0.00001) &&
            (fabs(a[i].z - b[i].z) <= 0.00001)) {
            flag = 1;
        }
    }
    return flag;
...
rVector *cMassCentroids(rVector *arrVec, size_t *numVectors, size_t NUMCLUSTERS, int *numThreads) {
    if (arrVec == NULL) {
    ...
    free(summator);
    FILE *wbFile;
    if ((wbFile = fopen("dynamic_Clusterization.txt", "wb")) == NULL) {
        fprintf(stderr, "Error: Failed to open file dynamic_Clusterization.txt!\n");
        return NULL;
    }
    size_t num;
    ...
    if (fclose(wbFile)) {
        fprintf(stderr, "Failed to close file dynamic_Clusterization.txt!\n");
        return NULL;
    }
    free(arrIndex);
    return centroid;
