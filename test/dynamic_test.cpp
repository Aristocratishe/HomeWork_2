...
TEST(DYNAMIC_TASK_TEST, BUILD_CLUSTERS_1) {
    int numThreads = sysconf(_SC_NPROCESSORS_CONF);
    if (numThreads == -1) {
        numThreads = 4;
    }
    ...
    rVector *centroids = cMassCentroids((rVector *)arrVec, &numVectors, NUMCLUSTERS, &numThreads);
    EXPECT_TRUE(centroids != NULL);
    free(centroids);
}

TEST(DYNAMIC_TASK_TEST, BUILD_CLUSTERS_2) {
    int numThreads = sysconf(_SC_NPROCESSORS_CONF);
    if (numThreads == -1) {
        numThreads = 4;
    }
